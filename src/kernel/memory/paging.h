...
typedef struct __attribute__((packed)) {
    u8_t present:1;        // [bit 0] present in RAM
    u8_t rw:1;             // [bit 1] read/write
    u8_t us:1;             // [bit 2] user/supervisor
    u8_t pwt:1;            // [bit 3] page-level write-through
    u8_t pcd:1;            // [bit 4] page-level cache-disabled
    u8_t accessed:1;       // [bit 5] accessed
    u8_t dirty:1;          // [bit 6] dirty
    u8_t pat:1;            // [bit 7] page attribute table, must be 0 unless PAT supported
    u8_t global:1;         // [bit 8] global translation
    u8_t _avl0:3;          // [bit 9-11]  available: ignored by cpu
    u32_t pf_address:20;   // [bit 12-31] physical base address of 4KB page frame
} pte_t;
...
typedef struct __attribute__((packed)) {
    ...
    u8_t pcd:1;            // [bit 4] page-level cache-disabled
    u8_t accessed:1;       // [bit 5] accessed
    u8_t _avl0:1;          // [bit 6] available: ignored by cpu
    u8_t ps:1;             // [bit 7] page size extension, 0=4KB 1=4MB, must be [0] for this struct
    u8_t _avl1:4;          // [bit 8-11]  available: ignored by cpu
    u32_t pt_address:20;   // [bit 12-31] physical address of 4KB aligned page table referenced by this entry
} pde_t;
...
typedef struct __attribute__((packed)) {
    ...
    u8_t accessed:1;       // [bit 5] accessed
    u8_t dirty:1;          // [bit 6] dirty
    u8_t ps:1;             // [bit 7] page size extension, 0=4KB 1=4MB, must be [1] for this struct
    u8_t global:1;         // [bit 8] global translation
    u8_t _avl0:3;          // [bit 9-11]  available: ignored by cpu
    u8_t pat:1;            // [bit 12] page attribute table, must be 0 unless PAT supported
    u16_t _pf_address:8;   // [bit 13-20] physical base address of 4MB page frame
    u8_t _reserved:1;      // [bit 21] reserved [0]
    u16_t pf_address:10;   // [bit 22-31] physical base address of 4MB page frame
} pde4mb_t;
